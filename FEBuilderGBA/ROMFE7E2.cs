...
{
    sealed class ROMFE7E2 : ROMFEINFO
    {
        public ROMFE7E2(ROM rom)
    	{
            VersionToFilename = "FE7E2";
            TitleToFilename = "FE7";
            ...
            unit_pointer = 0x09A5B4; //Ist das der richtige Pointer?
            unit_maxcount = 253; // ユニットの最大数
            unit_datasize = 52; // ユニットのデータサイズ
            max_level_address = 0x029C0A; //Ist das die richtige Addresse?
            max_luck_address = 0x029FF6; // Ist das die richtige Addresse?
            class_pointer = 0x017CF0; //Ist das der richtige Pointer?
            class_datasize = 84;  // ユニットのデータサイズ
            bg_pointer = 0xB9CC; //Ist das der richtige Pointer?
            portrait_pointer = 0x6C60; //Ist das der richtige Pointer?
            portrait_datasize = 28;
            icon_pointer = 0x4E5C; // Ist das der richtige Pointer?
            icon_orignal_address = 0x1E5190; // Das sollte die richtige Addresse sein.
            icon_orignal_max = 0xAC; // アイコンの最大数

            icon_palette_pointer = 0x4D78; // Ist das der richtige Pointer?
            unit_wait_icon_pointer = 0x0252A0; // Ist das der richtige Pointer?
            unit_wait_barista_anime_address = 0x025D50;  // Ist das die richtige Addresse?
            unit_wait_barista_id = 0x52;  // ユニット待機アイコンのバリスタの位置
            unit_icon_palette_address = 0x0D13D0; //Ist das die richtige Addresse?
            unit_icon_enemey_palette_address = 0x0D13F0; //Ist das die richtige Addresse?
            unit_icon_npc_palette_address = 0x0D1410; //Ist das die richtige Addresse?
            unit_icon_gray_palette_address = 0x0D1430; // Ist das die richtige Addresse?
            unit_icon_four_palette_address = 0x0D1450; // Ist das die richtige Addresse?
            unit_icon_lightrune_palette_address = 0x0D1470; // Ist das die richtige Addresse?
            unit_icon_sepia_palette_address = 0x0D1490; // Ist das die richtige Addresse?

            unit_move_icon_pointer = 0x06D5D0; // Ist das der richtige Pointer?
            lightrune_uniticon_id = 0x57; // ユニット(光の結界)のユニットアイコンのID
            map_setting_pointer = 0x031B9C;  // Ist das der richtige Pointer?
            ...
            map_config_pointer = 0x0195F0;      //Ist das der richtige Pointer?
            map_obj_pointer = 0x019658;         //Ist das der richtige Pointer?
            map_pal_pointer = 0x01968C;         //Ist das der richtige Pointer?
            map_tileanime1_pointer = 0x02D944;  //Ist das der richtige Pointer?
            map_tileanime2_pointer = 0x02E488;  //Ist das der richtige Pointer?
            map_map_pointer_pointer = 0x031BB8; //Ist das der richtige Pointer?
            map_mapchange_pointer = 0x031BD4;   //Ist das der richtige Pointer?
            map_event_pointer = 0x031BF0;       //Ist das der richtige Pointer?
            ...
            image_battle_animelist_pointer = 0x054144;   // Ist das der richtige Pointer?
            support_unit_pointer = 0xCA3F28;   // Ist das der richtige Pointer?
            support_talk_pointer = 0x078B78;   // Ist das der richtige Pointer?
            unit_palette_color_pointer = 0x0;  // ユニットのパレット(カラー)の開始位置
            unit_palette_class_pointer = 0x0;  // ユニットのパレット(クラス)の開始位置
            support_attribute_pointer = 0x026EC0;  //Ist das der richtige Pointer?
            terrain_recovery_pointer = 0x019F54; //Ist das der richtige Pointer?
            terrain_bad_status_recovery_pointer = 0x019F64; //Ist das der richtige Pointer?
            terrain_show_infomation_pointer = 0x08572C; //Ist das der richtige Pointer?
            ballista_movcost_pointer = 0x018BEC; //Ist das der richtige Pointer?
            ...
            map_terrain_name_pointer = 0x019F44; //Ist das der richtige Pointer?
            image_chapter_title_pointer = 0x0; //不明 章タイトルの開始位置
            image_chapter_title_palette = 0x4CB6F8; // Ist das die richtige Addresse?
            image_unit_palette_pointer = 0x054148; // Ist das der richtige Pointer?
            item_pointer = 0x0175DC; //Ist das der richtige Pointer?
            item_datasize = 36; // アイテムのデータサイズ
            item_effect_pointer = 0x052A4C; // Ist das der richtige Pointer?
            sound_table_pointer = 0x3F5C; // Ist das der richtige Pointer?
            sound_room_pointer = 0x01B88C; //Ist das der richtige Pointer?
            sound_room_datasize = 16; // サウンドルームのデータサイズ
            sound_room_cg_pointer = 0x0AB9E8; // Ist das der richtige Pointer?
            event_ballte_talk_pointer = 0x0793E8; //Ist das der richtige Pointer?
            event_ballte_talk2_pointer = 0x07955C; // Ist das der richtige Pointer?
            event_haiku_pointer = 0x079658; //Ist das der richtige Pointer?
            event_haiku_tutorial_1_pointer = 0x079664; // Ist das der richtige Pointer?
            event_haiku_tutorial_2_pointer = 0x079660; // Ist das der richtige Pointer?
            event_force_sortie_pointer = 0x08DFC4; // Ist das der richtige Pointer?
            event_tutorial_pointer = 0x0790E4; //Ist das der richtige Pointer?
            map_exit_point_pointer = 0x03A024; // Ist das der richtige Pointer?
            map_exit_point_npc_blockadd = 0x30; // arr[+0x30] からNPCらしい.
            map_exit_point_blank = 0x2B9734; // Ist das die richtige Addresse?
            sound_boss_bgm_pointer = 0x06822C; // Ist das der richtige Pointer?
            ...
            worldmap_event_on_stageselect_pointer = 0x0B60C8; //Ist das der richtige Pointer?
            ...
            item_cornered_pointer = 0x02A72C; //Ist das der richtige Pointer?
            ed_1_pointer = 0x0B8B0C;  //Ist das der richtige Pointer?
            ed_2_pointer = 0x0B8AE8;  //Ist das der richtige Pointer?
            ed_3a_pointer = 0xDB59FC;  //Ist das der richtige Pointer?
            ed_3b_pointer = 0xDB5A00;  //Ist das der richtige Pointer?
            ed_3c_pointer = 0xDB50F0;  //Ist das die richtige Addresse?
            generic_enemy_portrait_pointer = 0x76FC; //Ist das der richtige Pointer?
            ...
            cc_item_hero_crest_pointer = 0x027A40;  //Ist das der richtige Pointer?
            cc_item_knight_crest_pointer = 0x027A48;  //Ist das der richtige Pointer?
            cc_item_orion_bolt_pointer = 0x027A50;  //Ist das der richtige Pointer?
            cc_elysian_whip_pointer = 0x027A58;  //Ist das der richtige Pointer?
            cc_guiding_ring_pointer = 0x027A60;  //Ist das der richtige Pointer?
            cc_fallen_contract_pointer = 0x027AB4;  //Ist das der richtige Pointer?
            cc_master_seal_pointer = 0x027A68;  //Ist das der richtige Pointer?
            cc_ocean_seal_pointer = 0x027A8C;  //Ist das der richtige Pointer?
            cc_moon_bracelet_pointer = 0x027A80;  //Ist das der richtige Pointer?
            cc_sun_bracelet_pointer = 0x027A84;  //Ist das der richtige Pointer?
            unit_increase_height_pointer = 0x7364;  //Ist das der richtige Pointer? Irgendwas sieht hier seltsam aus.
            unit_increase_height_switch2_address = 0x7352; //Ist das die richtige Addresse?
            op_class_demo_pointer = 0xDADB80; //Ist das die richtige Addresse?
            op_class_font_pointer = 0x0;  //OPクラス日本語フォント
            op_class_font_palette_pointer = 0x0;  // OPクラス紹介フォントのパレット
            status_font_pointer = 0x6150;  //Ist das der richtige Pointer?
            ...
            bigcg_pointer = 0x0B761C; //Ist das der richtige Pointer?
            ...
            item_usability_array_pointer = 0x027238; //Ist das der richtige Pointer?
            item_usability_array_switch2_address = 0x027226; //Ist das die richtige Addresse?
            item_effect_array_pointer = 0x02D628;    //Ist das der richtige Pointer?
            item_effect_array_switch2_address = 0x02D60E; //Ist das die richtige Addresse?
            item_promotion1_array_pointer = 0x027968;   //Ist das der richtige Pointer?
            item_promotion1_array_switch2_address = 0x027956; //Ist das die richtige Addresse?
            item_promotion2_array_pointer = 0x095990;  //Ist das der richtige Pointer?
            item_promotion2_array_switch2_address = 0x095980; //Ist das die richtige Addresse?
            item_staff1_array_pointer = 0x027650;    //Ist das der richtige Pointer?
            item_staff1_array_switch2_address = 0x02763E; //Ist das die richtige Addresse?
            item_staff2_array_pointer = 0x067ED0;    //Ist das der richtige Pointer?
            item_staff2_array_switch2_address = 0x067EBE; //Ist das die richtige Addresse?
            item_statbooster1_array_pointer = 0x02D3C0;    //Ist das der richtige Pointer?
            item_statbooster1_array_switch2_address = 0x02D3AE; //Ist das die richtige Addresse?
            item_statbooster2_array_pointer = 0x0285CC;    //Ist das der richtige Pointer?
            item_statbooster2_array_switch2_address = 0x0285B8; //Ist das die richtige Addresse?
            item_errormessage_array_pointer = 0x027494;    //Ist das der richtige Pointer?
            item_errormessage_array_switch2_address = 0x027482;  //Ist das die richtige Addresse?
            event_function_pointer_table_pointer = 0xD878;    //Ist das der richtige Pointer?
            event_function_pointer_table2_pointer = 0x0;   //イベント命令ポインタ2 ワールドマップ
            item_effect_pointer_table_pointer = 0x055840;   //Ist das der richtige Pointer?
            command_85_pointer_table_pointer = 0x06792C;    //Ist das der richtige Pointer?
